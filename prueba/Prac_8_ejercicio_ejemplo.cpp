...
void InitGL ( GLvoid )     // Inicializamos parametros
{
	...
	t_espacio.LoadTGA("zamo.tga");//carga la textura
	t_espacio.BuildGLTexture();
	t_espacio.ReleaseImage();
}
...
void prisma (GLuint textura1, GLuint textura2)  //Funcion creacion prisma
{
	...
	GLfloat vertice [8][3] = {
		...
		glBegin(GL_POLYGON);	//Front
			...
			glTexCoord2f(0.0f, 0.0f); glVertex3fv(vertice[0]);//asignar vertices, coordenadas de textura
...
void display ( void )   // Creamos la funcion donde se dibuja
{
	...
	glTranslatef(pos_camX, pos_camY, pos_camZ);
		...
		glPushMatrix(); 
			...
			prisma(t_espacio.GLindex, t_metal01.GLindex);
		glPopMatrix();
