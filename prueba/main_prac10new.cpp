	...
	glPushMatrix();
		...
		glPushMatrix();		
			...
			glPushMatrix();
				...
				glRotatef(90, 0, 1, 0);
				glRotatef(90, 1, 0, 0);
				glRotatef(giroCalavera, 0, 0, 1);
				glScalef(0.10, 0.10, 0.10);

				glTranslatef(0, movKit, -50.0);
...
void animacion()
{
	...
	{
		printf("%.2f ", movKit);
		if (posZ == 125.0 && reversa==false) {
			giroCalavera = 180;
			movKit = (movKit-850.0);
		}

		if (posZ == -125.0 && reversa == true) {
			giroCalavera = 0;
			movKit = (movKit - 850.0);
		}
		...
		if (posZ >=-125.0 && posZ<126.0 && reversa==false) {
			posZ += 1.0;
			movKit += 3.4;
		}
		//printf("   %.2f", posZ);
