	...
	glRotatef(g_lookupdown,1.0f,0,0);
		...
		glPushMatrix();		
			glPushMatrix(); //Creamos cielo
				...
				fig1.skybox(800.0, 800.0, 800.0,text1.GLindex);
				glEnable(GL_LIGHTING);
			glPopMatrix();
			...
			glPushMatrix();
				...
				for (i = 0.0; i <= 35.0; i = i + 5.0) {
					for (j = 0.0; j <= 15.0; j = j + 5.0) {
							glPushMatrix();
								glTranslatef(i, 0, j);
								arbol_alpha();
							glPopMatrix();
...
void animacion()
{
	...
	if(circuito)
	{
		...
		if(recorrido3) //estado 3
		{
			rotKit = 160;
			rotTires = 90;
			movKitZ = movKitZ - 2*0.62;
			movKitX = movKitX + 2*0.25;
			movKitY = movKitY + 0.62;
			if(movKitZ<30) //-155
			{
				recorrido3 = false;
				recorrido4 = true;
		...
		if (recorrido5) //estado 5
		{
			...
			{
				recorrido5 = false;
				recorrido6 = true;//agregar otro = true;
			}
		}
		

		if(recorrido6) //estado 5 
		{
			rotKit = 0;
			rotTires = 0;
			movKitZ++;
